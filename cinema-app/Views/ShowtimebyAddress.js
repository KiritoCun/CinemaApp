...
export default function ShowtimeAddress({navigation, route, isLoggedIn}) {
  ...
  const ViewMovie = ({item, index}) => {
    ...
    // console.log(movie);
    return (
      <View style={{width: '100%', height: 300, marginBottom: '2%', borderBottomWidth: 0.2}}>
        <View style={{flexDirection: 'row', height: '40%',}}>
          <View style={{width: '22%', height: '100%', justifyContent: 'center', alignItems: 'center'}}>
            <Image source={{uri: item.posterUrl || "https://upload.wikimedia.org/wikipedia/commons/thumb/b/bc/Unknown_person.jpg/694px-Unknown_person.jpg"}}
              style={{width: '72%', height: '85%', resizeMode: 'stretch', borderRadius: 5}}
            />
          </View>
        ...
        <View style={{height: '55%', }}>
          <FlatList
            numColumns={4}
            data={getTimeById(dataTime, item.id)}
            renderItem={viewTime}
  ...
  const viewTime = ({item, index}) => {
    return (
      <TouchableOpacity style={{width: 82, height: 38, alignItems: 'center', justifyContent: 'center', borderWidth: 0.2, borderRadius: 5,
        ...
        onPress={() => {
          // selectMovie(movie);
          isLoggedIn ? navigation.navigate('Room', {}) : navigation.navigate('Login', {
          onLoginSuccess: () => {
            // Callback khi đăng nhập thành công, chuyển đến trang kế tiếp
            isLoggedIn = true;
            navigation.navigate('Room', {isLoggedIn, address});
        },
      })}}
      >
  ...
  return(
    <SafeAreaView style={styles.container}>
      ...
      <View style={{width: '100%', height: '6%', alignItems: 'center', borderBottomWidth: 7, borderColor: '#E5E5E5'}}>
        <FlatList
