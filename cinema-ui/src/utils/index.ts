...
export const formatTime = (time: string, option: string) => {
  ...
  if (diff < 30) {
    ...
    // less 1 hour
    return Math.ceil(diff / 60) + ' minute(s) ago';
  ...
  if (option) {
    ...
    return d.getMonth() + 1 + ' month(s)' + d.getDate() + ' day(s)' + d.getHours() + ' hour(s)' + d.getMinutes() + ' minute(s)';
  }
};
...
export const debounce = (func: any, wait: number, immediate: boolean) => {
  ...
  const later = function () {
    ...
    if (last < wait && last > 0) {
      ...
      timeout = null;
      // If it is set to immediate===true, there is no need to call because the start boundary has already been called
      if (!immediate) {
