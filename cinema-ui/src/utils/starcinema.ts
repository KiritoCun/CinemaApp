...
export function parseTime(time: any, pattern?: string) {
  ...
  const format = pattern || '{d}/{m}/{y} {h}:{i}:{s}';
  let date;
...
export const handleTree = <T>(data: any[], id?: string, parentId?: string, children?: string): T[] => {
  ...
  for (const d of data) {
    d.menuName = getRouteTitle(d.menuName);
...
// Verify that it is in blob format
export const blobValidate = (data: any) => {
  return data.type !== 'application/json';
};

// Verify container no format
export const containerValidate = (containerNo: string) => {
  ...
  if (re.test(containerNo)) {
    ...
    let sum = 0;
    const checkDigit = containerNo.substr(10);
