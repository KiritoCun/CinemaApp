<template>
  <el-card>
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="Column information" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          <el-table-column label="ID" type="index" min-width="5%" />
          <el-table-column label="Column name" prop="columnName" min-width="10%" :show-overflow-tooltip="true" />
          <el-table-column label="Description" min-width="10%">
            <template #default="scope">
              <el-input v-model="scope.row.columnComment"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="Column type" prop="columnType" min-width="10%" :show-overflow-tooltip="true" />
          ...
          <el-table-column label="Search mode" min-width="10%">
            <template #default="scope">
...
const submitForm = () => {
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(async res => {
    ...
    if (validateResult) {
      ...
      proxy?.$modal.msgError("The form validation failed, please check the submitted content again");
    }
  });
}
