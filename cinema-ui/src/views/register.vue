<template>
  <div class="register">
    <el-form ref="registerRef" :model="registerForm" :rules="registerRules" class="register-form">
      <h3 class="title">Eport VICT</h3>
      <el-form-item prop="tenantId" v-if="tenantEnabled">
        <el-select v-model="registerForm.tenantId" filterable placeholder="Please choose/Enter company name" style="width: 100%">
          <el-option v-for="item in tenantList" :key="item.tenantId" :label="item.companyName" :value="item.tenantId"> </el-option>
          <template #prefix><svg-icon icon-class="company" class="el-input__icon input-icon" /></template>
        </el-select>
      </el-form-item>
      <el-form-item prop="username">
        <el-input v-model="registerForm.username" type="text" size="large" auto-complete="off" placeholder="Account">
      ...
      <el-form-item prop="code" v-if="captchaEnabled">
        <el-input size="large" v-model="registerForm.code" auto-complete="off" placeholder="" style="width: 63%" @keyup.enter="handleRegister">
...
const registerRules: FormRules = {
    tenantId: [
        { required: true, trigger: "blur", message: "Please enter your tenant number" }
    ],
    username: [
    ...
    registerRef.value.validate(async (valid: boolean) => {
        if (valid) {
            ...
            if (!err) {
                ...
                await ElMessageBox.alert("<font color='red'>Congratulations, your account " + username + " has been successfully registered!</font>", "Alert", {
