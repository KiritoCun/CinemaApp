...
import vn.udn.dut.cinema.system.domain.bo.ShippinglineUserBo;
import vn.udn.dut.cinema.system.domain.bo.SysUserBo;
import vn.udn.dut.cinema.system.domain.vo.ShippinglineUserVo;
...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    @Override
    public TableDataInfo<ShippinglineUserVo> selectPageUserList(ShippinglineUserBo user, PageQuery pageQuery) {
        Page<ShippinglineUserVo> page = baseMapper.selectPageShippinglineUserList(pageQuery.build(), this.buildQueryWrapper(user));
        return TableDataInfo.build(page);
    }
    ...
    private Wrapper<SysUser> buildQueryWrapper(ShippinglineUserBo user) {
        Map<String, Object> params = user.getParams();
        QueryWrapper<SysUser> wrapper = Wrappers.query();
        wrapper.eq("u.del_flag", UserConstants.USER_NORMAL)
            .eq(ObjectUtil.isNotNull(user.getUserId()), "u.user_id", user.getUserId())
            ...
            .eq(StringUtils.isNotBlank(user.getStatus()), "u.status", user.getStatus())
            .eq(StringUtils.isNotBlank(user.getSystemType()), "u.system_type", user.getSystemType())
            .eq(StringUtils.isNotBlank(user.getOperatorCode()), "s.operator_code", user.getOperatorCode())
            .like(StringUtils.isNotBlank(user.getOperatorName()), "s.operator_name", user.getOperatorName())
            .like(StringUtils.isNotBlank(user.getPhonenumber()), "u.phonenumber", user.getPhonenumber())
            .between(params.get("beginTime") != null && params.get("endTime") != null,
                "u.create_time", params.get("beginTime"), params.get("endTime"))
            .and(ObjectUtil.isNotNull(user.getDeptId()), w -> {
    ...
    @Override
    public ShippinglineUserVo selectShippinglineUserById(Long userId) {
        return baseMapper.selectShippinglineUserById(userId);
    }
    ...
    @Override
    public boolean checkUserNameUnique(ShippinglineUserBo user) {
        boolean exist = baseMapper.exists(new LambdaQueryWrapper<SysUser>()
        	.eq(SysUser::getDelFlag, UserConstants.USER_NORMAL)
        	.eq(SysUser::getUserName, user.getUserName())
            .eq(SysUser::getSystemType, user.getSystemType())
    ...
    @Override
    public boolean checkPhoneUnique(ShippinglineUserBo user) {
        boolean exist = baseMapper.exists(new LambdaQueryWrapper<SysUser>()
        	.eq(SysUser::getDelFlag, UserConstants.USER_NORMAL)
            .eq(SysUser::getPhonenumber, user.getPhonenumber())
            .eq(SysUser::getSystemType, user.getSystemType())
    ...
    @Override
    public boolean checkEmailUnique(ShippinglineUserBo user) {
        boolean exist = baseMapper.exists(new LambdaQueryWrapper<SysUser>()
        	.eq(SysUser::getDelFlag, UserConstants.USER_NORMAL)
            .eq(SysUser::getEmail, user.getEmail())
            .eq(SysUser::getSystemType, user.getSystemType())
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int insertUser(ShippinglineUserBo user) {
        SysUser sysUser = MapstructUtils.convert(user, SysUser.class);
        // Add user information
        int rows = baseMapper.insert(sysUser);
        user.setUserId(sysUser.getUserId());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int updateUser(ShippinglineUserBo user) {
        // Added user and role management
        insertUserRole(user, true);
        SysUser sysUser = MapstructUtils.convert(user, SysUser.class);
        // Prevent accidental deletion of data after an incorrect update
    ...
    public int updateUserProfile(SysUserBo user) {
        return baseMapper.update(null,
            new LambdaUpdateWrapper<SysUser>()
                ...
                .set(StringUtils.isNotEmpty(user.getPhonenumber()) && !user.getPhonenumber().contains("*"), SysUser::getPhonenumber, user.getPhonenumber())
                .set(StringUtils.isNotEmpty(user.getEmail()) && !user.getEmail().contains("*"), SysUser::getEmail, user.getEmail())
    ...
    private void insertUserRole(ShippinglineUserBo user, boolean clear) {
        this.insertUserRole(user.getUserId(), user.getRoleIds(), clear);
    }
